...
#            self.__plannedNumberOfAssessments = "Missing"
    ...
    def getRegion(self):
        ...
        if (self.getTrialType() == 'D' or self.getTrialType() == 'R') and (self.getIndication() in ['H','F','I','S']): 
            if (self.tptIdKey.strip()[4:7] == "EUR") and (self.tptIdKey.strip()[11:12] == 'R'):
                region = "EMEA"
...
#        pattern = re.sub(r'^\DR|D\d+EUR.*R$', '', self.tptIdKey)   
    ...
    def getCropUseGroup(self):
        if((self.crops is None) or (len(self.crops)<1)):
            return None
        else:
            return self.listToString(self.getNestedValue(self.crops, 'useGroupCode'))
        ...
        for element in self.plannedAssessments:
            ...
            for key, value in element.items():
                if (isinstance(value, dict)):
                    ...
                    value = list(value.values())[0]  # The assessments have always a single crop/target value
...
#        requiredFields = dfWeightsFiltered.index.values.tolist()
        ...
        if len(orderedAssessments)<1:
            orderedAssessments = self.emptyAssessments()
...
#            dfFiltered = df.loc[(df["Status"] == 0) & (df["Trial"] == self.getTrialType())  & (df["Indication"] == self.getIndication()) & (df["Region"] == self.getRegion()) ]
            dfFiltered = df.loc[df["Case"] == "TD 1"]
